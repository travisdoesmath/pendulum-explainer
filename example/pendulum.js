class Pendulum {
    ...
    RK4(dt, thetas, thetaDots) {
        let k1 = this.lagrange_rhs(thetas, thetaDots);
        let k2 = this.lagrange_rhs(thetas.map((x, i) => x + 0.5*dt*k1[0][i]), thetaDots.map((x, i) => x + 0.5*dt*k1[1][i]));
        let k3 = this.lagrange_rhs(thetas.map((x, i) => x + 0.5*dt*k2[0][i]), thetaDots.map((x, i) => x + 0.5*dt*k2[1][i]));
        let k4 = this.lagrange_rhs(thetas.map((x, i) => x +     dt*k3[0][i]), thetaDots.map((x, i) => x +     dt*k3[1][i]));
